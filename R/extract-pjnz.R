...
extract_pjnz <- function(pjnz = NULL, dp_file= NULL, pjn_file = NULL){
  ...
  v$netmigrhiv <- get_dp_netmigrhiv(dp, proj_years)
...
combine_inputs <- function(lst){
  ...
  nm <- c("totpop", "births", "netmigr",  "hivpop", "infections", "artpop",
          "art15plus_num", "art15plus_needart", "age14hivpop", "netmigrhiv")
...
get_dp_netmigrhiv <- function(dp, proj_years) {
  dn <- list(agegr = agegr_labels(), sex = c("male", "female"), year = proj_years)
  netmigrhiv <- as.numeric(unlist(dpsub(dp, "<HIVMigrantsByAgeSex MV>", 2+1:(NG*AG), 3+seq_along(proj_years))))
  netmigrhiv <- array(netmigrhiv, c(AG, NG, length(proj_years)), dn)
  netmigrhiv
}
